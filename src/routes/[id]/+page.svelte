<script>
  ...
  <main class="flex flex-col justify-center items-center p-10 gap-5 prose text-center mx-auto bg-muted rounded max-w-7xl lg:mx-auto lg:w-[calc(100%-4rem)]">
    ...
    <h1 class="font-black text-4xl">{data.page.title}</h1>
    <p class="text-xs prose prose-sm font-medium w-72">{@html data.page.description}</p>
    
    {#if data.page.headerImg}
      <img src="https://api.nafuna.tv/api/files/pages/{data.page.id}/{data.page.headerImg}" alt="Header" class="p-5 rounded"/>
    {/if}
    <div class="rounded-lg overflow-hidden">
      <p class="prose prose-sm max-w-prose">{@html data.page.Content}</p>
    </div>
